...
use frontend\modules\bkhealth\models\FileUploadSearch;
...
class FileUploadController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            $model->debit_file = $model->upload($model, 'debit_file');
            $model->upload_date = date('Y-m-d h:m:s');
            $model->save();
            
            Yii::$app->session->setFlash('success', 'อับโหลดไฟล์สำเร็จ--> ให้กดตามประเภทไฟล์ เพื่อผนวกข้อมูล');
